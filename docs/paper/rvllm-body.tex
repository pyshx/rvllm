...
\begin{center}
  ...
  \vspace{0.3em}
  {\normalsize San Francisco, April 1, 2026\par}
\end{center}
\vspace{1em}
...
is a 26-package Rust workspace with 54 CUDA kernels, FA3 v3 cp.async + split-KV
attention, no-fallback kernel validation, and a Rust-native PTX compiler that
generates fused decode kernels 2--7.5$\times$ faster than the hand-written CUDA
baselines in the measured microbenchmarks. In the latest verified apples-to-apples
...
and 2{,}723~tok/s over HTTP at 200 requests / concurrency 32 (\textbf{0.95$\times$ stock vLLM}).
An intermediate Python stack, \texttt{rvllm-lite}, retains near-stock direct-engine
speed but falls to 131.8~tok/s over HTTP, isolating the serving layer as a major
source of practical overhead. The remaining direct-engine gap is dominated by GEMM
tuning, attention microarchitecture, and scheduler maturity rather than by Rust or
CUDA integration overhead. rvLLM is open-source under the Apache-2.0 license.
\end{abstract}
...
\begin{enumerate}
  \item A modular 26-package Rust architecture for LLM inference, with GPU-resident
  ...
  \item 54~CUDA kernels including FlashAttention-3 v3 with cp.async bulk copies and split-KV,
        paged KV cache support, GPU argmax, fused residual-RMSNorm, and reserve-based
        safe-max VRAM sizing for server startup. No-fallback startup validation ensures
        every required kernel loads or the server fails fast.
  \item Verified three-way H100 benchmark results against stock vLLM and an
        intermediate Python serving layer, separating direct-engine performance from
