...
\begin{center}
  ...
  \vspace{0.3em}
  {\normalsize San Francisco, April 4, 2026\par}
\end{center}
\vspace{1em}
...
microbenchmarks. In the latest honest same-GPU comparison on H100~SXM~80\,GB
serving Qwen2.5-7B f16 against vLLM~0.19.0 (April 2026), rvLLM reaches
4{,}407.5~tok/s at $N\!=\!32$ (\textbf{0.99$\times$ vLLM}),
8{,}038.0~tok/s at $N\!=\!64$ (\textbf{1.01$\times$ vLLM}), and
13{,}110.1~tok/s at $N\!=\!128$ (\textbf{0.94$\times$ vLLM}), while remaining
materially behind at $N\!=\!1$ (133.1 vs.\ 165.5~tok/s, \textbf{0.80$\times$}).
The most important changes in this revision are architectural rather than cosmetic:
batch-1 normal decode now defaults to the reusable batched scratch path, and batched
execution now uses an explicit hybrid GEMM policy with QKV/O/down on cuBLAS or
cublasLt and GateUp+SiLU on CUTLASS. rvLLM is open-source under the Apache-2.0
license.
\end{abstract}
...
All direct-engine runs in Table~\ref{tab:h100_direct} use 128 output tokens per
request and compare current committed rvLLM directly against current vLLM~0.19.0
on the same GPU and model snapshot.

\subsection{Direct Engine: Current vLLM 0.19.0 Comparison}

\begin{table}[t]
  \centering
  \caption{Current H100 direct-engine comparison. Qwen2.5-7B f16, 128 output tokens/request.}
  \label{tab:h100_direct}
  \small
  \begin{tabular}{@{}rrrr@{}}
    \toprule
    \thead{$N$} & \thead{vLLM~0.19.0} & \thead{rvLLM} & \thead{rvLLM / vLLM} \\
    \midrule
    1   & 165.5 & 133.1 & 0.80$\times$ \\
    \rowcolor{gray!8}
    32  & 4{,}467.7 & 4{,}407.5 & 0.99$\times$ \\
    64  & 7{,}972.1 & 8{,}038.0 & 1.01$\times$ \\
    \textbf{128} & \textbf{13{,}903.5} & \textbf{13{,}110.1} & \textbf{0.94$\times$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
The most important finding of this revision is that the previous public comparison
was distorted by two architecture problems inside rvLLM rather than by one deep
fundamental deficit.

\begin{enumerate}
  \item \textbf{Batch-1 default path selection was wrong.} Normal $T\!=\!1$ decode
        had already left the older fused default, but it was still staying on the
        legacy single-token family instead of the reusable \texttt{Batched} path.
        Fixing that lifted the current normal batch-1 path to 133.1~tok/s.
  \item \textbf{The batched GEMM policy was only half-expressed.} The runner wanted
        a hybrid strategy, but the actual dispatch did not encode that cleanly, so
        QKV could still opportunistically route through CUTLASS merely because the
        shared library was present.
\end{enumerate}

The current batched policy is now explicit:
\begin{itemize}
  \item QKV: cuBLAS / cublasLt
  \item O-proj: cuBLAS / cublasLt
  \item GateUp+SiLU: CUTLASS
  \item Down-proj: cuBLAS / cublasLt
\end{itemize}

\subsection{Interpretation}

Table~\ref{tab:h100_direct} changes the story materially. rvLLM is no longer
uniformly behind by $\sim$10\%. Instead, the picture is split:
\begin{itemize}
  \item \textbf{Batch-1 remains weak}: 0.77$\times$ vLLM at $N\!=\!1$.
  \item \textbf{The main batched regime is nearly there}: 0.99$\times$ at $N\!=\!32$
        and effectively 1.00$\times$ at $N\!=\!64$.
  \item \textbf{Large batched decode still trails slightly}: 0.95$\times$ at $N\!=\!128$.
\end{itemize}

This implies that the remaining gap is not one monolithic systems-language tax. The
normal batched stack is now competitive; the largest remaining problems are the
single-stream decode path and a few percent of high-end batched efficiency.

\subsection{Historical HTTP Serving Context}

\begin{table}[t]
  \centering
  \caption{Historical H100 HTTP comparison retained for context only.}
  \label{tab:h100_http}
  \small
  \begin{tabular}{@{}lrrrr@{}}
    \toprule
...
These HTTP numbers were measured against an older vLLM release and are kept only
because they remain useful for separating direct-engine performance from
serving-stack performance. They are not the current headline comparison in this
revision.
...
\begin{table}[t]
  \centering
  \caption{Measured VRAM footprint for the older H100 HTTP comparison retained for context.}
  \label{tab:h100_vram}
  \small
  \begin{tabular}{@{}lrr@{}}
    \toprule
...
The remaining gap to stock vLLM is now more specific than in earlier revisions.
The latest H100 run suggests that rvLLM's batched direct-engine path is already
competitive, while single-stream decode and a few percent of high-end batched
efficiency still need work. The measured gains in rvLLM come from a compound
effect of several reinforcing mechanisms:
...
\begin{enumerate}
  ...
  \item \textbf{Correct path selection}: batch-1 normal decode now takes the reusable
        batched scratch route instead of the older single-token default family.
  \item \textbf{Explicit batched hybrid GEMM routing}: the best current policy keeps
        QKV/O/down on cuBLAS while using CUTLASS only where it currently wins
        (GateUp+SiLU).
\end{enumerate}

The direct-engine table shows that these mechanisms are sufficient to make rvLLM
effectively tie vLLM at $N\!=\!64$ and nearly tie it at $N\!=\!32$, but not yet
enough to win at $N\!=\!1$ or $N\!=\!128$.
...
\begin{itemize}
  ...
  \item rvLLM still trails stock vLLM materially on direct engine at $N\!=\!1$,
        and remains a few percent behind at $N\!=\!128$.
...
reach near-parity with stock vLLM in the batched direct-engine regime on a current
H100 apples-to-apples benchmark: 0.99$\times$ at $N\!=\!32$, effectively
1.00$\times$ at $N\!=\!64$, and 0.95$\times$ at $N\!=\!128$. The same benchmark
also makes the remaining weakness explicit: single-stream decode is still only
0.77$\times$ vLLM at $N\!=\!1$.

The most important result of this revision is that the last large public gap was
partly architectural self-inflicted damage. Correcting the batch-1 default path
selection and expressing the batched hybrid GEMM policy explicitly moved the
comparison from a uniform ``rvLLM is slower'' story to a split picture: batched
decode is competitive, single-stream decode is not.
