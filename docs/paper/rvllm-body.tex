...
microbenchmarks. In an honest same-GPU comparison on H100~SXM~80\,GB
serving Qwen2.5-7B f16 against the latest vLLM release (0.19.0, April 2026),
rvLLM reaches 4{,}385~tok/s at $N\!=\!32$ direct engine
(\textbf{0.92$\times$ stock vLLM~0.19.0}, which achieves 4{,}773~tok/s).
The gap is consistent at $\sim$10\% across all concurrency levels ($N\!=\!1$
through $N\!=\!32$), pointing to GEMM algorithm selection as the dominant
remaining bottleneck: vLLM uses Triton autotuned GEMMs with months of
per-model config tuning, while rvLLM uses cuBLAS with freshly-implemented
disk-cached autotuning. rvLLM is
